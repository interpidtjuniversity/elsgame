...
//
	...
	//
	lxy[0] = Sprite::create("3.png");
	PhysicsBody *itsbody = PhysicsBody::createBox(Size(lxy[0]->getContentSize().width, lxy[0]->getContentSize().height));
	lxy[0]->setPosition(100, 500);
	lxy[0]->setPhysicsBody(itsbody);
	this->addChild(lxy[0]);
	lxy[1] = Sprite::create("1.png");
	lxy[1]->setPosition(200, 500);
	PhysicsBody *itsbody1 = PhysicsBody::createBox(Size(lxy[1]->getContentSize().width, lxy[1]->getContentSize().height));
	lxy[1]->setPhysicsBody(itsbody1);
	this->addChild(lxy[1]);
	auto moveTo = MoveTo::create(1, Vec2(lxy[0]->getPositionX() + 100, lxy[0]->getPositionY() + 200));
	lxy[0]->runAction(moveTo);
	removeChild(lxy[0], true);
	lxy[0] = Sprite::create("10.png");
	lxy[0]->setPosition(500, 500);
	this->addChild(lxy[0]);
	//
