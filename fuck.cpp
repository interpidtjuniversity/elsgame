...
bool MainScene2::init()
{
	...
	for (int i = 0; i < LINE; i++)
	{
		for (int j = 0; j <COLUME; j++)
		{
			...
			m_pSquare[i][j]->setPosition(Director::sharedDirector()->convertToGL(ccp(i * 20 + i * 2 + 10, j * 20 + j * 2 + 10)));//
...
//	removeChild(hero1,true);
	...
	m_pSquare[0][0]->setColor(ccc3(0, 255, 0));
	m_pSquare[0][1]->setColor(ccc3(0, 255, 0));
	//
